...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        if iteration >= epoch_step:
            break
        images, texts, depth_images = batch
        with torch.no_grad():
            if cuda:
                images  = images.cuda(local_rank)
                depth_images = depth_images.cuda(local_rank)
        ...
        if not fp16:
            ...
            logits_per_image, _                 = model_train(images, texts, depth_images)#clip.forward(),返回特征相乘的矩阵，数值表示图像与文本匹配的相似度
            logits_per_text                     = logits_per_image.t()
            #对角线部分即为正样本，将其值设为1，其余部分为0
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            if cuda:
                images  = images.cuda(local_rank)
                depth_images = images.cuda(local_rank)

            optimizer.zero_grad()
    
            logits_per_image, _                 = model_train(images, texts, depth_images)
            logits_per_text                     = logits_per_image.t()
            labels                              = torch.arange(len(logits_per_image)).long().to(images.device)
            loss_logits_per_image               = nn.CrossEntropyLoss()(logits_per_image, labels)
            loss_logits_per_text                = nn.CrossEntropyLoss()(logits_per_text, labels)
