        ...
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for iteration, batch in tqdm(enumerate(self.gen_val)):
                ...
                with torch.no_grad():
                    ...
                    image_depth_features = self.net.encode_depth_image(depth_images)
                    idep_features.append(image_depth_features)
            ...
            idep_features = torch.cat(idep_features, 0)
            
            i_features  = i_features / i_features.norm(dim=-1, keepdim=True)
            t_features  = t_features / t_features.norm(dim=-1, keepdim=True)
            idep_features = idep_features / idep_features.norm(dim=-1, keepdim=True)

            i_features = i_features + 0.3 * idep_features
