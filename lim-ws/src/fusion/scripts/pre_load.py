...
def load_points():
#获取保存在goal.txt中的 在map坐标系下的导航目标点
    room_list = []
    with open('/home/yang/lim-ws/src/fusion/scripts/pose/small_house/goals.txt', 'r') as f:
        for line in f:
            ...
            point_pose.pose.orientation.z = float(data[2])
            point_pose.pose.orientation.w =float(data[3])
...
#到达目标点后获取检测结果，提取距离信息创建关联模型的训练集
    ...
    #将关联结果保存 用于后续训练
    with open('/home/yang/lim-ws/src/fusion/scripts/train_corelate/small_house/train_model.txt', 'a+') as file:
        for k in range(n):
